...
app.get('/api/stats', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT key, value FROM system_stats WHERE key IN (?, ?)', ['discord_members', 'telegram_members']);
        const stats = {
            discord_members: 0,
            telegram_members: 0
        };
...
// --- Facebook OAuth Flow ---
const FACEBOOK_APP_ID = (process.env.FACEBOOK_APP_ID || '949236510997168').trim();
const FACEBOOK_APP_SECRET = (process.env.FACEBOOK_APP_SECRET || '90d640b8cc494fb02dc5334650db9550').trim();
